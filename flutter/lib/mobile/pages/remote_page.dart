...
import '../../utils/foldable_helper.dart';
...
class _RemotePageState extends State<RemotePage> with WidgetsBindingObserver {
            ...
            () => getRawPointerAndKeyBody(Overlay(
              initialEntries: [
                OverlayEntry(builder: (context) {
                  // Pixel Fold / foldable adaptive layout:
                  // On wide screens (>=580dp shortest side) in landscape,
                  // use side-panel layout with remote desktop left & controls right.
                  final isWideLandscape = FoldableHelper.shouldUseSidePanel(context);
                  return Container(
                    color: kColorCanvas,
                    child: isWebDesktop
  ...
  /// Pixel Fold inner screen landscape layout.
  ///
  /// When the device is unfolded and in landscape orientation
  /// (e.g. Pixel Fold inner 2208×1840), show the remote desktop
  /// on the left and a persistent toolbar panel on the right.
  Widget _buildFoldableWideLayout() {
    return Row(
      children: [
        // Remote desktop canvas (takes most of the width)
        Expanded(
          flex: 73,
              ...
              child: Column(
                children: [
                  ...
                  _sideToolbarButton(
                    ...
                    onTap: () => clientClose(sessionId, gFFI),
                  ),
                  _sideToolbarButton(
                    icon: Icons.keyboard,
                  ...
                  _sideToolbarButton(
                    ...
                    onTap: () => showOptions(context, widget.id, gFFI.dialogManager),
                  ),
                  _sideToolbarButton(
                    icon: gFFI.ffiModel.touchMode
                        ? Icons.touch_app
                        : Icons.mouse,
                  ...
                  _sideToolbarButton(
                    ...
                    onTap: () => onPressedTextChat(widget.id),
                  ),
                  _sideToolbarButton(
                    icon: Icons.more_vert,
                    label: translate('More'),
                    onTap: () => showActions(widget.id),
                  ),
                  const Spacer(),
                  // Quality monitor at bottom
                  Padding(
  ...
  /// Build a single button for the foldable side toolbar.
  Widget _sideToolbarButton({
