...
class MainActivity : FlutterActivity() {
    companion object {
        ...
        var foldableMethodChannel: MethodChannel? = null
    ...
    override fun configureFlutterEngine(flutterEngine: FlutterEngine) {
        ...
        // Pixel Fold / foldable device detection channel
        foldableMethodChannel = MethodChannel(
            flutterEngine.dartExecutor.binaryMessenger,
            "com.carriez.flutter_hbb/foldable"
        )
        initFoldableChannel(foldableMethodChannel!!)
    ...
    /// Pixel Fold / foldable device support.
    /// Detects foldable hardware and monitors fold state changes
    /// (folded, unfolded, half-opened).
    private fun initFoldableChannel(channel: MethodChannel) {
        val hasFoldFeature = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            packageManager.hasSystemFeature("android.software.screen_fold") ||
            packageManager.hasSystemFeature("android.hardware.type.foldable")
        } else {
            false
        }
        debugPrint("initFoldableChannel", "hasFoldFeature=$hasFoldFeature")

        channel.setMethodCallHandler { call, result ->
            when (call.method) {
                "isFoldable" -> {
                    result.success(hasFoldFeature)
                }
                "getFoldPosture" -> {
                    result.success(getCurrentFoldPosture())
                }
                else -> {
                    result.error("-1", "No such foldable method", null)
                }
            }
        }

        // Listen for fold state changes on API 29+
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q && hasFoldFeature) {
            setupFoldStateListener(channel)
        }
    }

    @RequiresApi(Build.VERSION_CODES.Q)
    private fun getCurrentFoldPosture(): String {
        // Try Jetpack WindowManager-based detection
        try {
            val display = windowManager.defaultDisplay
            // On Pixel Fold and similar devices, the display width changes
            // when unfolding. We use this as a proxy for fold state.
            val metrics = DisplayMetrics()
            display.getRealMetrics(metrics)
            val shortestSide = Math.min(metrics.widthPixels, metrics.heightPixels)
            debugPrint("getFoldPosture", "shortestSide=$shortestSide")

            return when {
                // Pixel Fold inner screen: 1840px shortest → unfolded
                shortestSide >= 1400 -> "unfolded"
                // Small cover screen → folded
                shortestSide <= 1200 -> "folded"
                else -> "halfOpened"
            }
        } catch (e: Exception) {
            Log.e(logTag, "getFoldPosture error: ${e.message}")
        }
        return "folded"
    }

    @RequiresApi(Build.VERSION_CODES.Q)
    private fun setupFoldStateListener(channel: MethodChannel) {
        // Use DisplayListener to detect fold/unfold transitions
        // on API 29+ devices.
        // Pixel Fold changes display metrics when folding/unfolding.
        val displayManager = getSystemService(Context.DISPLAY_SERVICE) as android.hardware.display.DisplayManager
        var lastPosture = getCurrentFoldPosture()
        ...
        displayManager.registerDisplayListener(
            object : android.hardware.display.DisplayManager.DisplayListener {
                ...
                override fun onDisplayChanged(displayId: Int) {
                    val newPosture = getCurrentFoldPosture()
                    if (newPosture != lastPosture) {
                        debugPrint("foldState", "$lastPosture -> $newPosture")
                        lastPosture = newPosture
                        channel.invokeMethod("onFoldStateChanged", newPosture)
                    }
                }
            },
            null
        )
        debugPrint("foldStateListener", "registered, initial posture=$lastPosture")
    }

    private fun debugPrint(tag: String, msg: String) {
        Log.d("${logTag}_foldable", "[$tag] $msg")
    }
